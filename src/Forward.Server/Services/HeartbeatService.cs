    ...
    public class HeartbeatService : IDisposable
    {
        ...
        private async Task CheckHeartbeat(string cmd, TcpClient client, CancellationToken token)
        {
            ...
            try
            {
                ...
                {
                    var index = await stream.ReadAsync(buff, 0, buff.Length, token).TimeoutAfter(TimeSpan.FromMilliseconds(_receiveTimeout), token);
                    if (index <= 0) break;

                    await stream.WriteAsync(_okBuff, 0, _okBuff.Length, token).TimeoutAfter(TimeSpan.FromMilliseconds(_sendTimeout), token);
                }
            }
            catch (Exception ex)
            {
